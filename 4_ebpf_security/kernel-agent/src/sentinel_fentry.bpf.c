/**
 * SentinelEdge eBPF Kernel Security Monitor (fentry version)
 *
 ...
 * - File system operations (VFS open calls using fentry)
 */
...
#include <bpf/bpf_endian.h>   // Enable bpf_ntohs/bpf_htons functions
...
int trace_execve(struct trace_event_raw_sys_enter *ctx)
{
    ...
    // Get syscall parameter filename
...
int trace_tcp_connect(struct pt_regs *ctx)
{
    ...
    // Compatible with new kernel fields
...
/* ========== 3) File access monitoring (fentry version) ========= */
SEC("fentry/vfs_open")
int BPF_PROG(trace_file_open_fentry, struct path *path, struct file *file)
{
